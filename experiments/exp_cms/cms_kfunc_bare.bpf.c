...
/* Dichiarazione diretta della kfunc, senza mykperf_module.h:
 * cosi' non tiriamo dentro nessuna delle strutture del framework. */
extern __u64 bpf_mykperf__rdpmc(__u8 counter) __ksym;
...
SEC("xdp")
int cms_kfunc_bare(struct xdp_md *ctx)
{
...
DROP:
    {
        ...
        /* impedisce al compilatore di eliminare le letture come dead-code,
         * identico al pattern usato in cms_native.bpf.c */
        if (end == start)
            return XDP_ABORTED;
    }
    return XDP_DROP;
