...
#include "cms_native.skel.h"

int if_index;
struct cms_native_bpf *cms_native;

void sig_handler(int sig)
{
    bpf_xdp_detach(if_index, 0, NULL);
    cms_native_bpf__destroy(cms_native);
    exit(0);
}
...
int main(int argc, char **argv)
{
    ...
    cms_native = cms_native_bpf__open_and_load();
    if (!cms_native) {
    ...
    err = bpf_xdp_attach(if_index, bpf_program__fd(cms_native->progs.cms_native), 0, NULL);
    if (err) {
